...
with st.sidebar:  
    ...
    system_message = st.text_area(  
        "システムメッセージを入力してください",  
        value="あなたは親切なAIアシスタントです。ユーザーの質問に簡潔かつ正確に答えてください。",  
        height=100  
    )  
    ...
    # 空欄のチャットを除外  
    sidebar_messages = [chat for chat in st.session_state.sidebar_messages if chat.get("first_assistant_message")]  
    ...
    for i, chat in enumerate(sidebar_messages):  
        if chat and "first_assistant_message" in chat:  
            ...
            if st.button(keyword, key=f"history_{i}"):  
                st.session_state.main_chat_messages = chat["messages"]  
                st.session_state.current_chat_index = len(st.session_state.sidebar_messages) - 1 - i  # インデックスを調整  
                st.rerun()  
...
if prompt := st.chat_input("What is up?"):  
    ...
    messages.extend([{"role": m["role"], "content": m["content"]} for m in st.session_state.main_chat_messages[-(num_messages_to_include + 1):]])  
...
if st.session_state.current_chat_index is not None:  
    st.session_state.main_chat_messages = st.session_state.sidebar_messages[st.session_state.current_chat_index]["messages"]  
